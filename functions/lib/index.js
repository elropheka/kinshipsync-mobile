...
exports.sendPushNotification = exports.dailyEventCountdowns = exports.sendUpcomingEventReminders = exports.sendChatMessageNotification = exports.processEventInvitationFCM = void 0;
    ...
    try {
        ...
        for (let i = 0; i < tickets.length; i++) {
            ...
            if (ticket.status === "error") {
                functions.logger.error(`Failed to send Expo notification to token ${validTokens[i]}:`, ticket.message);
            }
        }
    ...
    try {
        ...
        if (response.failureCount > 0) {
            response.responses.forEach((resp, idx) => {
                if (!resp.success) {
                    functions.logger.error(`Failed to send FCM to token ${tokens[idx]}:`, resp.error);
...
exports.processEventInvitationFCM = functions.firestore
    .document("pending_event_invitations_fcm/{invitationId}")
    .onCreate(async (snapshot) => {
    const invitationData = snapshot.data();
    const { invitedGuestEmail, invitedGuestName, organizerId, eventId, eventName, } = invitationData;
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                if (invitedUserData.fcmTokens && invitedUserData.fcmTokens.length > 0) {
                    ...
                    functions.logger.info(`Sent FCM ${fcmType} to guest: ${invitedUserData.displayName} (${invitedGuestEmail})`);
                    ...
                    functions.logger.info(`Created in-app ${fcmType} for guest: ${invitedUserData.displayName}`);
                }
                else {
                    functions.logger.warn(`Invited guest ${invitedGuestEmail} found, but has no FCM tokens.`);
                }
            }
            else {
                functions.logger.info(`Invited guest with email ${invitedGuestEmail} is not a registered user or email not found.`);
            }
        }
        catch (error) {
        ...
        try {
            ...
            if (organizerDoc.exists) {
                ...
                if (organizerData.fcmTokens && organizerData.fcmTokens.length > 0) {
                    ...
                    const organizerNotificationBody = `You have successfully invited ${invitedGuestName} to your event: "${eventName}".`;
                    const organizerFcmData = {
                        eventId: eventId,
                        guestName: invitedGuestName,
                        type: "guest_invited_confirmation",
                    };
                    await sendPushNotificationToTokens(organizerData.fcmTokens, organizerNotificationTitle, organizerNotificationBody, organizerFcmData);
                    functions.logger.info(`Sent FCM confirmation to organizer: ${organizerData.displayName} (ID: ${organizerId})`);
                }
                else {
                    functions.logger.warn(`Organizer ${organizerId} found, but has no FCM tokens.`);
                }
            }
            else {
...
exports.sendChatMessageNotification = functions.firestore
    ...
    functions.logger.info(`New message in conversation ${conversationId} from ${senderId}. Sending notifications.`);
    ...
    try {
        ...
        if (conversationData.participants && Array.isArray(conversationData.participants)) {
            ...
            functions.logger.warn(`Conversation ${conversationId} using 'participantIds' field. Consider migrating.`);
        }
        ...
        functions.logger.error("Error sending chat message notifications for conversation " + conversationId + ":", error);
        return null;
    }
});
// --- Scheduled Function for Upcoming Event Reminders ---
exports.sendUpcomingEventReminders = functions.pubsub
...
exports.dailyEventCountdowns = functions.pubsub.schedule("every 24 hours").onRun(async (_context) => {
 ...
 */
exports.sendPushNotification = functions.https.onCall(async (request, _context) => {
    const { recipientId, title, body, notificationData } = request.data;
    if (!recipientId || !title || !body) {
        throw new functions.https.HttpsError("invalid-argument", "The function must be called with 'recipientId', 'title', and 'body'.");
    }
    try {
...
__exportStar(require("./mailjet"), exports);
