...
// Define the interface for the data payload expected by the sendSMS function
interface SendSMSData {
  toPhoneNumber: string; // Phone number in E.164 format (e.g., +1234567890)
  message: string; // SMS message content (max 1600 characters for concatenated SMS)
  fromPhoneNumber?: string; // Optional sender phone number (uses config default if not provided)
}
 ...
 */
export const sendSMS = functions.https.onCall(async (data: SendSMSData, _context) => {
  ...
  if (!toPhoneNumber || !message) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "The function must be called with \"toPhoneNumber\" and \"message\"."
    );
  }
  ...
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Twilio credentials are not configured. Please set them using firebase functions:config:set twilio.account_sid=\"YOUR_SID\" twilio.auth_token=\"YOUR_TOKEN\" twilio.phone_number=\"YOUR_NUMBER\"."
    );
  }
  ...
  if (!senderPhoneNumber) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Twilio phone number is not configured and no fromPhoneNumber was provided. Please set twilio.phone_number or provide fromPhoneNumber."
    );
  }
